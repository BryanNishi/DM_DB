...
class Trap extends Component {
  ...
  generateHandler = event => {
    ...
    for (i = 0; i < 5; i++) {
      //pull random trap
      let traps =
        this.state.traps[Math.floor(Math.random() * this.state.traps.length)] +
        " causes... ";
      //pull random tripp
      let tripped =
        this.state.tripped[
          Math.floor(Math.random() * this.state.tripped.length)
        ] + " and then... ";
      //pull random hazard
      let hazard =
        this.state.hazard[
          Math.floor(Math.random() * this.state.hazard.length)
        ] + ". The party notices...";
      //pull random disarm
      let disarm =
        this.state.disarm[
          Math.floor(Math.random() * this.state.disarm.length)
        ] + " that disarms the trap.";
      ...
      <div className="trapGenerator">
        ...
        <List>
          {this.state.trapNames.map(trap => (
            <ListItem key={trap}>{trap}</ListItem>
          ))}
        </List>
        <Button
