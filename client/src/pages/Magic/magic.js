...
class Magic extends Component {
  ...
  generateItemHandler = event => {
    ...
    for (i = 0; i < 5; i++) {
      //pull random name
      let randomItem = this.state.itemArray[
        Math.floor(Math.random() * this.state.itemArray.length)
      ];
    ...
    return (
      <div className="magicItemGenerator">
        ...
        <Row className="buttonList">
          <Button
            onClick={this.generateItemHandler}
            name={this.state.generate.magicItem}
          />
        </Row>
      </div>
    );
