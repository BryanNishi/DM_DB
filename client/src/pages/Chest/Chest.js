...
class Chest extends Component {
  ...
  generateHandler = event => {
    ...
    for (i = 0; i < 5; i++) {
      ...
      let style =
        "A " +
        this.state.style[Math.floor(Math.random() * this.state.style.length)] +
        " styled chest is made of ";
      //pull random tripp
      let material =
        this.state.material[
          Math.floor(Math.random() * this.state.material.length)
        ] + ", trimmed with ";
      //pull random hazard
      let trim =
        this.state.trim[Math.floor(Math.random() * this.state.trim.length)] +
        ", and decorated with ";
      //pull random disarm
      let decor =
        this.state.decor[Math.floor(Math.random() * this.state.decor.length)] +
        ". ";
      //pull random disarm
      let trap =
        "It has a " +
        this.state.trap[Math.floor(Math.random() * this.state.trap.length)] +
        " trap with a ";
      //pull random disarm
      let lock =
        this.state.lock[Math.floor(Math.random() * this.state.lock.length)] +
        " lock. ";
      //pull random disarm
      let key =
        "The Key seems to be " +
        this.state.key[Math.floor(Math.random() * this.state.key.length)];
      ...
      <div className="chestGenerator">
        ...
        <List>
          {this.state.chestNames.map(chest => (
            <ListItem key={chest}>{chest}</ListItem>
          ))}
        </List>
        <Button
