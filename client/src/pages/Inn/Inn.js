...
class Inn extends Component {
  ...
  generateHandler = event => {
    ...
    for (i = 0; i < 5; i++) {
      ...
      let adj =
        "The " +
        this.state.adjs[Math.floor(Math.random() * this.state.adjs.length)] +
        " ";

      //pull random noun
      let noun =
        this.state.nouns[Math.floor(Math.random() * this.state.nouns.length)] +
        " ";

      //pull random type
      let type = this.state.types[
        Math.floor(Math.random() * this.state.types.length)
      ];
